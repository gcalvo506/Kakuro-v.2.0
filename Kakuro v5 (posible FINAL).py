...
def juego():
    ...
    btn_jugada=Button(juegoraiz, bg='steel blue', text='Borrar\nJugada', font=('Console',20),command=lambda:borrar_jugada(),
    ...
    btn_record=Button(juegoraiz, bg='steel blue', text='TOP\n10', font=('Console',20),
                       width=7, bd='2', relief=RAISED).place(x=1130,y=400)
...
def aceptar_timer():
    ...
    """f=open("kakuro2018configuración.dat","r")
    lista_config=f.readline()
    lista_config=eval(lista_config)
    lista_config+=[tiempo_total]
    f.close()
    f=open("kakuro2018configuración.dat","w")
    f.write(str(lista_config))
    f.close()
    print(tiempo_total)"""
...
def acerca():
    ...
    Label(acerca,text="Nombre del Programa: Kakuro.py",
    ...
    Label(acerca,text="Fecha de Creación: 11/04/2018.",
